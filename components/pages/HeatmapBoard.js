...
export default function HeatmapBoard({ id }) {
    ...
    const data = async (url) => { return await getImage(`/screenshot/${websiteId}/all`, {
        url: url,
        })
    };

     const initHeatmap = async values => {
      data(values.url).then(data => {setImageObjectURL(URL.createObjectURL(data.data))});
        ...
        for (const point of points) {
          ...
          const duplicate = unique.find(
            (obj) => obj.posX === item.posX && obj.posY === item.posY
          );
          if (!duplicate) {
              item.count = 1
              unique.push(item);
        ...
        console.log(unique)
        console.log(Math.max(...unique.map(o => o.count)))

        heatmapInstance.setData({
          max: Math.max(...unique.map(o => o.count)),
          data: unique.map(x => {return  {x: x.posX, y: x.posY, value: x.count}}) 
        });
     }
    }
    ...
    function handleClose() {
     ...
     return (
      ...
      <PageHeader>
        ...
        {optionHeatmap && (
        <Modal title={<FormattedMessage id="label.heatmap-filter" defaultMessage="Heatmap Filter" />}>
        ...
        <div style={{height: "37vw", width:"100%", position: "relative"}} className="App">
            <img src={imageObjectURL} style={{position: "absolute", left:0, top: 0, width: "100%"}} id="imageId"/>
        </div>
        }
