...
function parseFilters(table, column, filters = {}, params = [], sessionKey = 'session_id') {
  const { domain, url, event_url, referrer, os, browser, device, country, event_name, query, step } =
    filters;

  const pageviewFilters = { domain, url, referrer, query };
  const sessionFilters = { os, browser, device, country };
  const eventFilters = { url: event_url, event_name, step: step };
...
async function executeQuery(query, params = []) {
  ...
  return prisma.$executeRawUnsafe.apply(prisma, [sql, ...params]);
}
